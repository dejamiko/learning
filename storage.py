...
class ObjectStorage:
    ...
    def __init__(self, c, oracle, env):
        """
        Initialize the object storage
        :param c: the configuration object
        :param oracle: the oracle object
        ...
        self.generate_objects(TrajectoryObject)
    ...
    def generate_objects(self, object_class):
        ...
        for i in range(self.c.OBJ_NUM):
            objects.append(object_class(i, np.random.uniform(0, 1, self.c.LATENT_DIM), -1, self.c))
    ...
    def generate_helper_data(self):
        ...
        for i in known_objects:
            self.object_demos[i] = self.oracle.get_demo(self.env, self.objects[i], self.c)
